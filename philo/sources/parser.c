...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/04/26 09:27:55 by rantario          #+#    #+#             */
/*   Updated: 2022/04/26 10:01:49 by rantario         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	parser(int argc, char **argv, t_philo *philo)
{
	...
	if (!philo->num_philos || !philo->time_die || \
		!philo->time_eat || !philo->time_sleep || !philo->num_time_eat)
		return (ft_error(7));
	if (overflow)
		return (ft_error(9));
	return (0);
